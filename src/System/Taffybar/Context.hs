...
import           System.Taffybar.Information.X11DesktopInfo (X11Context, X11PropertyT, withX11Context, withX11EventLoop, postX11RequestSyncProp, doLowerWindow, getAtom)
...
import           UnliftIO.MVar (MVar, newMVar, modifyMVar, modifyMVar_, readMVar)
...
initContext cfg = do
  ...
  listeners <- newMVar []
  contextState <- newMVar M.empty
  x11ContextVar <- using getDefaultX11Context >>= newMVar
  windowsVar <- newMVar []
  pure Context
    { x11ContextVar
    , listeners
...
lowerX11Window :: MonadUnliftIO m => Context -> Gtk.Window -> m ()
lowerX11Window context window = runX11Context_ context $ void $ runMaybeT $ do
    gdkWindow <- MaybeT $ Gtk.widgetGetWindow window
    x11Window <- MaybeT $ liftIO $ Gtk.castTo X11Window gdkWindow
    xid <- GdkX11.x11WindowGetXid x11Window
...
refreshTaffyWindows = mapReaderT postGUIASync $ do
  ...
  modifyMVar_ windowsVar rebuildWindows
...
asksContextVar :: MonadIO m => (r -> MVar b) -> ReaderT r m b
asksContextVar getter = asks getter >>= readMVar

-- | Run a function needing an X11 connection in 'TaffyIO'.
runX11 :: MonadIO m => X11PropertyT m a -> Taffy m a
runX11 action =
...
runX11Def :: MonadUnliftIO m => a -> X11PropertyT m a -> Taffy m a
runX11Def dflt prop = runX11 $ postX11RequestSyncProp prop dflt

runX11Context :: MonadUnliftIO m => Context -> a -> X11PropertyT m a -> m a
runX11Context context dflt prop =
  runTaffy context (runX11Def dflt prop)

runX11Context_ :: MonadUnliftIO m => Context -> X11PropertyT m () -> m ()
runX11Context_ = flip runX11Context ()
...
putState getValue = do
  ...
  modifyMVar contextVar $ \contextStateMap ->
...
-- | Variant of 'managed_' for resources which are acquired in a
-- 'ReaderT' monad.
managedR_ :: MonadManaged m => (forall r. ReaderT x IO r -> ReaderT x IO r) -> ReaderT x m ()
managedR_ f = do
  x <- ask
  managed_ (\g -> runReaderT (f (liftIO g)) x)
...
startX11EventHandler = do
  run <- asks runTaffy
  lift . managed_ $ withX11EventLoop def
    (void . lift . run . handleX11Event)
...
unsubscribe identifier = do
  ...
  modifyMVar_ listenersVar $ return . filter ((== identifier) . fst)
...
subscribeToAll listener = do
  ...
  modifyMVar_ listenersVar (return . addListener)
  return identifier
