...
import Data.Maybe (maybeToList, fromMaybe, mapMaybe, isJust, isNothing)
...
specLog' :: MonadIO m => Int -> String -> m ()
specLog' level msg = do
  v <- getVerbosity
  when (level <= v) $ liftIO $ T.putStrLn (T.pack msg)

getVerbosity :: MonadIO m => m Int
getVerbosity = fromMaybe 0 . (>>= readMaybe) <$> lookupEnv "TAFFYBAR_TEST_VERBOSE"
...
streamSpecCond :: Int -> Int -> StreamSpec any ()
streamSpecCond level verbosity = if verbosity >= level then inherit else nullStream

setStderrCond :: Int -> ProcessConfig i o e -> ProcessConfig i o ()
setStderrCond = setStderr . streamSpecCond 1
...
withXserver prog args display action = do
  cfg <- makeXvfb <$> getVerbosity
  specLog $ "withXserver running: " ++ show cfg
  withProcessTerm cfg $ \p ->
    consumeXDisplayFd (getStdout p) >>= action
  where
